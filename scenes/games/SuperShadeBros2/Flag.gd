...
func _on_FlagArea_area_entered(area):
	if area.name == "ShadeArea":
		if area.global_position != $Marker2D.global_position:
			ShadeBro = area.get_parent()
			$Tween.interpolate_property(ShadeBro, "global_position", ShadeBro.global_position, $Marker2D.global_position, 1, 0)
